...
public class K8sService {
	...
	public void createPod(CoffeePod cPod) {
		...
		// metadata
		PodBuilder pb = new PodBuilder().withNewMetadata().withName(cPod.getName())
		...
		SpecNested<PodBuilder> specNested = pb.withNewSpec().addNewContainer()
				...
				.withResources(null)
				//
				.addNewEnv().withName("POD_IP").editOrNewValueFrom().editOrNewFieldRef().withFieldPath("status.podIP").endFieldRef().endValueFrom().endEnv()
				//
				.addNewEnv().withName("MASTER_IP").withValue("xx.xx.xx.xx").endEnv()
				//
				.addNewVolumeMount().withName(K8sCsts.CONFIG_DIR).withMountPath(K8sCsts.CONFIG_DIR_CONTAINER_PATH)
				.endVolumeMount()
				//
